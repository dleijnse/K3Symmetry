...
def f(x, y, z):
    return 0.4 - 0.7 * (x**2 + y**2 + z**2) + 1.6 * x*y*z - 1.3 *  (x**4 + y**4 + z**4) + 1.3 * (x**2 * y**2 + x**2 * z**2 + y**2 * z**2)
...
def plot_marching(circular=False):
    delta = 0.5
    bound = 1
    steps = m.ceil(bound * 2 / delta)
    ...
    if (circular):
        M = 0.3
        # We set F(X, Y, Z) to the value of M if the coordinat point has radius > bound.
        D = dist2(X, Y, Z)
        # print(D <= bound)
        print(F)
        F2 = np.where(D <= (bound ** 2), F, M)
        print(F2)
    else:
        F2 = F

    if False:
        vX = np.transpose(verts)[0]
        vY = np.transpose(verts)[1]
        vZ = np.transpose(verts)[2]
        D = dist2(vX, vY, vZ)
        verts = verts[D <= (bound ** 2)]

        NumVerts = len(verts)
        facesX = np.transpose(faces)[0]
        facesY = np.transpose(faces)[1]
        facesZ = np.transpose(faces)[2]
        M = np.maximum(facesX, np.maximum(facesY, facesZ))   
        faces = faces[M < NumVerts]
        
    verts, faces, normals, values = measure.marching_cubes(F2, 0, spacing=[np.diff(xs)[0]]*3)
    verts -= 3
